    ...
    public class MongoCollectionTests
    {
        ...
        public void TestAggregateExplain()
        {
            if (_primary.Supports(FeatureId.AggregateExplain))
            {
                ...
                Assert.True(response.Contains("stages") || response.Contains("queryPlanner")); // #6 AggregateExplain
            }
        }
...
#pragma warning restore
        ...
        public void TestCreateIndex()
        {
            ...
            AssertNamespace(indexes[0]); // #5 IndexInfo.Namespace
...
#pragma warning restore
        ...
        {
            RequireServer.Check().VersionLessThan(new SemanticVersion(4, 9, 0, "")); // #8 GeoHaystack
...
#pragma warning restore
        ...
        public void TestMapReduce()
        {
            ...
            Assert.True(result.Ok);
            //Assert.True(result.Duration >= TimeSpan.Zero); // #4 MapReduce
            //Assert.Equal(9, result.EmitCount); // #4 MapReduce
            //Assert.Equal(5, result.OutputCount); // #4 MapReduce
            //Assert.Equal(3, result.InputCount); // #4 MapReduce
        ...
        {
            if (_primary.InstanceType == MongoServerInstanceType.StandAlone) // #9 ReIndex
            {
                _collection.Drop();
                _collection.Insert(new BsonDocument("x", 1));
                _collection.Insert(new BsonDocument("x", 2));
