...
namespace MongoDB.Driver.Core.Operations
{
    public class GeoSearchOperationTests : OperationTestBase
    {
        ...
        public void Execute_should_return_expected_result(
            ...
            bool async)
        {
            RequireServer.Check().ClusterTypes(ClusterType.Standalone, ClusterType.ReplicaSet).VersionLessThan(new SemanticVersion(4, 9, 0, "")); // #4 Geo
            EnsureTestData();
        ...
        public void Execute_should_throw_when_maxTime_is_exceeded(
            [Values(false, true)] bool async)
        {
            RequireServer.Check().Supports(Feature.FailPoints).ClusterTypes(ClusterType.Standalone, ClusterType.ReplicaSet).VersionLessThan(new SemanticVersion(4, 9, 0, "")); // #4 Geo
            var subject = new GeoSearchOperation<BsonDocument>(
                _collectionNamespace,
                new BsonArray { 1, 2 },
                BsonDocumentSerializer.Instance,
